...
const AppHeader = ({ activeRoute, onNavigate }: AppHeaderProps) => {
  ...
  const formattedCreatorBalance = new Intl.NumberFormat('en-US', {
    maximumFractionDigits: 0,
  }).format(balance);

  return (
    <>
      <header className={`flex items-center justify-between bg-[#f2f0f8] border-b border-border relative z-50 ${isCompactHeader ? 'px-10 py-4' : 'px-6 py-3'}`}>
        {/* Left side - Logo and Nav */}
        <div className={`flex items-center ${isCompactHeader ? 'gap-14' : 'gap-8'}`}>
          <button
            type="button"
          ...
          <nav className={`flex items-center ${isCompactHeader ? 'gap-12' : 'gap-6'}`}>
            {navRoutes.map((item) => (
              <button
                key={item.route}
                type="button"
                onClick={() => onNavigate(item.route)}
                className={`${isCompactHeader ? 'text-[1.1rem]' : 'text-base'} font-semibold leading-none transition-colors ${
        ...
        <div className="flex items-center gap-4">
          {isConnected ? (
            isCompactHeader ? (
              <>
                <div className="flex items-center rounded-full bg-[#ece8f4] p-1 shadow-sm">
                  <div className="flex items-center gap-3 rounded-full px-4 py-2.5 min-h-[52px]">
                    ...
                    {isBalanceLoading ? (
                      ...
                      <span className="text-[1.05rem] font-semibold text-[#6d34b1] tracking-tight">
                        {showBalances ? `${formattedCreatorBalance} REEF` : '•••••• REEF'}
                      </span>
                    )}
                  </div>
                  <button
                    type="button"
                    className="rounded-full bg-gradient-to-r from-[#a93185] to-[#5d3bad] text-white text-[1.05rem] font-semibold px-8 py-2.5 min-h-[52px] hover:brightness-110 transition-all"
                ...
                <button
                  ...
                  className="flex items-center justify-center h-[52px] w-[52px] rounded-full bg-[#ece8f4] text-[#a33e91] hover:bg-[#e7e1f2] transition-colors"
                  onClick={() => setShowAccountModal(true)}
                >
            ...
            <Button
              ...
              className={`bg-gradient-to-r from-[#a93185] to-[#5d3bad] text-white ${isCompactHeader ? 'rounded-full px-7 py-2.5 text-[1.02rem]' : 'rounded-[12px] px-6'} hover:scale-105 hover:shadow-lg hover:brightness-110 active:scale-95 transition-all duration-200 ease-out`}
            >
              Connect Wallet
            </Button>
          )}
